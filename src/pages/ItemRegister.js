...
const ItemRegister = () => {
    ...
    const fetchItemRegister = async () => {
        try {
            const response = await axios.post('http://43.201.115.178:5000/create_item',{
                user_id: 1,
                name: name,
                price: price,
    ...
    const test = async () => {
        try {
            const response = await axios.get('https://lg029tjlp1.execute-api.ap-northeast-2.amazonaws.com/dev/netzero-12-labmda-test'           
            )
            console.log(response.data);
